...
static void segment_write(const struct sseg_digit_data *ddata)
{
	...
	value = SFR_BYTE(iomap->segment_port);
	value &= (uint8_t)~enable_mask;
	value |= enable_set;
	SFR_BYTE(iomap->segment_port) = value;
}
		    ...
		    char digit)
{
	...
	for (i = SSEG_A; i < SSEG_NR_SEGMENTS; i++) {
		if (segment_mask & BITMASK8(i))
			segment_enable_set |= iomap->segment_masks[i];
	}
...
void sseg_init(struct sseg_digit_data *ddata)
{
	...
	for (i = 0; i < SSEG_NR_SEGMENTS; i++)
		segment_mask |= iomap->segment_masks[i];
	...
	segment_write(ddata);
	SFR_BYTE(iomap->segment_ddr) |= segment_mask;
